...
Four runnable, end-to-end examples live under `examples/`, one per nws
shape. Each folder is self-contained: `run.sh` sets up **the current
folder** as a full nws workspace, and its README documents the exact flags,
the generated flake, and the "edit the clone, rebuild" proof.

- `examples/overlay-ros` — the classic overlay workspace (reworked to the
  current-folder style): ROS 1 `noetic` on
  [nix-ros-overlay](https://github.com/lopsided98/nix-ros-overlay), two
  TurtleBot repos cloned in and spliced, an nws-generated `nix develop` ROS
  devShell, and the gazebo GUI layer.
- `examples/overlay-flakes` — an overlay **whose packages are themselves
  nix flakes**: the overlay flake is `hyprwm/hyprlang`, and splicing in the
  cloned repo inherits its dependencies (`hyprutils`) from the overlay's own
  flake-input wiring.
- `examples/overlay-python` — a python overlay: nixpkgs's own
  `python312Packages` as the package set, the `requests` and
  `typing_extensions` modules cloned and spliced in, plus a managed
  devShell (`black`) generated by nws.
- `examples/flake-program-lib` — the **flake backend** (no overlay):
  program + library — `hyprcursor` + `hyprlang`, both real flakes, the
  program's flake declaring the library as an input — with nws wiring the
  **sibling-input override** so the program builds against the local clone.

### Running the examples

Every `run.sh` follows the same contract:

- **Your own empty folder.** Run from a folder you own — never from inside
  the nws repo (the scripts refuse, so the repo's own `flake.nix` can never
  be clobbered). The script clones into the current folder, copies the
  example's resolver in as `./nws-resolver.sh`, and registers the folder
  with nws. It creates or deletes nothing, never unregisters, and tears
  nothing down — the registered workspace (and its generated `flake.nix`)
  is the outcome:

  ```bash
  mkdir demo && cd demo
  bash <nws-repo>/examples/overlay-ros/run.sh   # swap in the example you want
  ```

- **`nws` must be installed.** The scripts take the binary from the `$NWS`
  environment override if set, else from `PATH` (`command -v nws`), and
  fail with an install hint when it's missing — see
...
- **Re-running** in the same folder is safe: registration is skipped when
  ...
  point the workspace at **different example parameters**, a re-register
  does not update the old config — unregister it first, then re-run:
