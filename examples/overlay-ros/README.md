...
`/tmp/nws-example-overlay-ros-daemon.log`); if the spawned daemon does not
come up within the readiness window the script **fails hard with the log
...
To keep the spawned daemon alive so it keeps watching the folder (handy
while you iterate on the clones):
